...
def solve_milp_model(employees_df, tasks_df, hours_per_day=8, tau_penalty=20, time_weight=500):
    ...
    if status in [pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE]:
        ...
        # --- NEW EXTRACTION: We need the actual hours worked! ---
        assignments = []
        for (i, j) in valid_pairs:
            hours = x[(i, j)].solution_value()
            if hours > 0:
                assignments.append({
                    'Employee_ID': i,
                    'Task_ID': j,
        ...
        # Return assignments_df instead of None
        return metrics, schedule_df, assignments_df 
